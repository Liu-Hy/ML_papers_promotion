...
![不同度量下的重心对比](DD_figs/WMDD_different_metrics.jpg)

_不同分布度量方式的对比：Wasserstein 重心（右）相比 KL 散度和 MMD（左、中），能更好地保留原始分布的几何结构特征。_
...
![WMDD方法流程图](DD_figs/WMDD_method_diagram.jpg)

_WMDD 的整体流程：真实数据和合成数据通过特征网络提取特征，计算 Wasserstein 重心，然后通过特征匹配和 PCBN 正则共同优化合成样本。_
...
![WMDD生成的合成图像](DD_figs/WMDD_distilled_images.jpg)

_WMDD 在 ImageNet-1K 上生成的合成图像示例（10 IPC）：每张图像都捕捉到了对应类别的关键视觉特征。_
...
![GUARD生成的合成图像](DD_figs/GUARD_distilled_images.jpg)

_GUARD 在 ImageNet-1K 上生成的合成图像示例（1 IPC）：即使每类只有一张图像，也能捕捉类别的核心语义特征，并且具有对抗鲁棒性。_
